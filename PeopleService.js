...
function getPerson(personId) {
  try {
    ...
    for (var j = 0; j < found.headers.length; j++) {
      var key = String(found.headers[j]).trim();
      if (!key) key = 'col_' + j;
      var val = (j < found.rowData.length) ? found.rowData[j] : '';
...
function updatePerson(personId, patch) {
  try {
    ...
    for (var k = 0; k < keys.length; k++) {
      ...
      if (headerIdx !== -1) {
        sheet.getRange(rowIndex, headerIdx + 1).setValue(newVal);
        continue;
      }

      // PERSON_COL_MAP（CRM追加列）フォールバック
      if (PERSON_COL_MAP.hasOwnProperty(colName)) {
        sheet.getRange(rowIndex, PERSON_COL_MAP[colName] + 1).setValue(newVal);
      }
      // 見つからない列名は無視（エラーにしない）
    }
...
function _findPersonRow(personId) {
  ...
  for (var s = 0; s < PERSON_SHEETS.length; s++) {
    ...
    // person_id列をヘッダから動的検出（見つからなければデフォルト16）
    var personIdColIdx = PERSON_EXTRA_COL.PERSON_ID;
    for (var hp = 0; hp < headers.length; hp++) {
      if (String(headers[hp]).trim() === 'person_id') { personIdColIdx = hp; break; }
    }

    for (var i = 1; i < data.length; i++) {
      var cellId = (personIdColIdx < data[i].length) ? String(data[i][personIdColIdx]).trim() : '';
...
function createPerson(payload) {
  try {
    ...
    if (PERSON_SHEETS.indexOf(sheetName) === -1) {
      throw new Error('不正な区分です。モデル / インフルエンサー / スポーツ選手・著名人 のいずれかを指定してください。');
    }
    var ss = getSpreadsheet();
    var sheet = ss.getSheetByName(sheetName);
